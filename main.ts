...
async function main(students: Student[]) {
  do {
    ...
    if (check.check2 === "Log In") {
      ...
      const isNameValidExisting = students.some(
        (student) => student.name === enteredName
      );
      const isNameValidNew = newStudents.some(
        (student) => student.name === enteredName
      );
